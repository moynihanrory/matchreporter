...
from constants import TIDY_SCX_CODE, TIDY_SCX_OBS_1, TIDY_SCX_OBS_2, TIDY_SCX_OBS_3, TIDY_SCX_START, TIDY_SCX_END, \
...
def extractHalf(row, lastRow):
    ...
    if lastRow[src.constants.FORMAT_ROW_HALF] == 2:
        return lastRow[src.constants.FORMAT_ROW_HALF]

    code = row[TIDY_SCX_CODE].upper().startswith(src.constants.TIDY_SCX_OBS_HT)
...
def extractSector(half, time):
    ...
    timeBucket = src.helpers.timeHelper.getTimeBucket(minute, half)
